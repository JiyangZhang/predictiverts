...
def cloneProject(project, example_number, defects4j_bin=DEFECTS4J_BIN, \
    ...
    if project == "time":
        sub.run(defects4j_bin + ' checkout -p %s -v %sb -w Joda_%s-%s' \
            % (project.title(), example_number, project.title(), example_number), shell=True, \
            stdout=open(os.devnull, 'w'), stderr=sub.STDOUT)
    else:
        sub.run(defects4j_bin + ' checkout -p %s -v %sb -w apache_%s-%s' \
            % (project.title(), example_number, project.title(), example_number), shell=True, \
            stdout=open(os.devnull, 'w'), stderr=sub.STDOUT)
...
def runRTSTool(tool, project, example_number, version, results_dir=_RESULTS_DIR):
    ...
    elif tool == "seq2pred":  # the name of the tool is a Macro :)
        # Step0: TODO: train the model, prepare data
        # Step1: do inference on buggy version
        with IOUtils.cd(f"{Macros.python_dir}"):
            BashUtils.run(f"./run.sh test_seq2pred Ekstazi-simple {project}")
        # end with
        # Step2: run the test to see which are
    elif tool == 'clover':
